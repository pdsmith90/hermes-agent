    ...
    def _track_tool_override_policy(self, manifest: PluginManifest, module_name: str) -> None:
        ...
        with replacement_coordinator.transaction():
            ...
            current_policy = _registry.register_plugin_override_policy(
                # Speculative: caches the policy at load time for EVERY plugin, whether or not it ever
                # overrides a tool. audit=False keeps this routine bookkeeping out of the capability_check
                # audit trail so a real denied override (register_tool) stays visible in it.
                module_name, PluginContext(manifest, self)._tool_override_allowed("", audit=False), scope=scope,
            )
