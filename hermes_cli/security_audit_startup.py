...
def _sshd_config_unreadable() -> list[str]:
    ...
    return [str(p) for p in _sshd_config_paths() if p.is_file() and not os.access(p, os.R_OK)]
...
def _ssh_password_auth_enabled() -> Optional[str]:
    ...
    if directives and directives[-1] == "no":
        return None
    if not directives and _sshd_config_unreadable():
        # The directive may live in a file we cannot read (a root-only drop-in); asserting
        # "enabled by default" would be a guess, so say nothing.
        return None
