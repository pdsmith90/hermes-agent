...
def plugin_capability_granted(
    plugin_id: str, capability: str, config: Optional[Mapping[str, Any]] = None, *, audit: bool = True,
) -> bool:
    ...
    ``audit=False`` withholds the ``capability_check`` audit line for SPECULATIVE callers — those
    asking "would this be allowed?" to cache a policy, not because a plugin attempted the
    privileged action. The plugin loader probes ``tools.override`` once per plugin per load, and
    that probe wrote a ``decision=deny`` line indistinguishable from a genuine refusal, burying
    real denials in bookkeeping. Enforcement sites keep the default: a refusal leaves a record.
    """
