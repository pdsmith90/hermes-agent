...
def _get_process_hermes_home() -> Path:
    ...
    Under pytest with HERMES_HOME wiped, this returns a throwaway directory rather than the
    platform default. The platform default IS the operator's live ~/.hermes:
    ``test_connect_websocket_sets_channel_ua_tag`` reached it through a ``clear=True`` environ
    patch and wrote the running gateway's ``gateway_state.json``, leaving a phantom
    ``feishu: connected`` platform entry that survived gateway restarts. Production behaviour is
    unchanged — neither pytest signal is ever true outside a test run."""
    if not os.environ.get("HERMES_HOME", "").strip() and _running_under_pytest():
        global _PYTEST_FALLBACK_HERMES_HOME
        if _PYTEST_FALLBACK_HERMES_HOME is None:
            _PYTEST_FALLBACK_HERMES_HOME = Path(tempfile.mkdtemp(prefix="hermes-pytest-home-"))
