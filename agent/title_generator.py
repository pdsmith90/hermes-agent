...
def generate_title(
    ...
    try:
        ...
        response = call_llm(
            ...
            # Trailing `/no_think` is Qwen's soft-switch: reasoning models under `--reasoning auto`
            # otherwise spend their whole reasoning budget on a 3-7 word title and blow past the
            # request timeout, so titles silently never generate. Inert text for other models.
            messages=[{"role": "system", "content": prompt}, {"role": "user", "content": f"{user_snippet}\n\n/no_think"}],
