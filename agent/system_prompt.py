...
def _guidance_parts(agent: Any) -> List[str]:
    ...
    #
    # Cron is excluded: a scheduled run has no interactive user and no steer is ever delivered to
    # it (the scheduler never calls apply_pending_steer_*). Describing the channel there is pure
    # downside — it hands an unattended agent both the marker syntax and the rule that marker text
    # carries "the same authority as their original request", with nothing that can legitimately
    # produce one. On 2026-08-13 a cron agent emitted a forged marker as its OWN assistant output
    # and obeyed it, creating two cron jobs via `hermes cron create` that no job prompt authorized.
    if (agent.platform or "").lower().strip() != "cron":
