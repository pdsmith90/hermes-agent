...
The prompt-injection scanner stopped the run before the agent started. Open the saved report under `~/.hermes/cron/output/<job-id>/`; its `Scanner source` label points to the job prompt/per-run context, script output, monitor data, upstream job output, notepad, or attached skill. The report intentionally omits matched text. Review that component and remove or rephrase the triggering instruction; do not disable the scanner. If it says `combined assembled prompt`, review all inputs assembled for the job together.
