...
class TelegramFallbackTransport(httpx.AsyncBaseTransport):
    ...
    async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
        ...
        # A pooled socket going stale between getUpdates long-poll cycles is routine and
        # self-healing (Telegram's edge closes idle keep-alives), so per-attempt WARNINGs turned
        # ordinary recycling into ~13 lines/day that the nightly error triage mined as the box's
        # largest error source. Attempts log at DEBUG; escalation happens ONCE, after the loop,
        # only when every path failed and the request is genuinely about to fail.
        ...
        for ip in self._attempt_order():
            ...
            try:
                ...
                if ip is None:
                    ...
                    logger.debug("[Telegram] Dual-stack api.telegram.org path failed (%s)", failure)
                    continue
                logger.debug("[Telegram] IPv4 Telegram API IP %s failed: %s", ip, failure)
