...
def _resolve_cron_disabled_toolsets(cfg: dict) -> list[str]:
    ...
    See #25752.
    """
    # NOTE: "memory" is deliberately NOT here, reverting upstream 03dc4aad5 ("hide memory tool
    # from cron agents") on this fork. "memory" is not only the built-in tool's toolset:
    # memory_provider_tools_enabled treats it as an explicit disable of the EXTERNAL provider
    # tools too, and fact_store/fact_feedback are the only memory tools a cron job has. Adding it
    # here removed fact writing from every cron agent on 2026-08-09. The unbacked built-in
    # memory() is the lesser problem: the cron prompts tell the model it does not work in cron.
    ...
    # code_execution: only when it cannot possibly succeed. Under approvals.cron_mode=deny (the
    # default) tools/approval.py refuses execute_code outright — no user is present to approve —
    # so offering it hands the model a tool guaranteed to fail (daily-review burned its whole
    # 80-call budget against that wall on 2026-08-09). Gated rather than unconditional: with
    # cron_mode=approve the tool genuinely works. The code_execution toolset contains ONLY
    # execute_code, so unlike a "memory" entry this cannot strip anything else.
    try:
        from tools.approval_context import _get_cron_approval_mode
        if _get_cron_approval_mode() == "deny":
            disabled.append("code_execution")
    except Exception:
        logger.debug("cron: could not resolve approval mode; leaving code_execution exposed", exc_info=True)
...
def _report_gate_missing_marker(job: dict, final_response: str) -> Optional[str]:
    ...
    A cron run ends on the first text turn that carries no tool call, whatever that text says.
    On 2026-09-06 experiment-design ended 25 minutes of real work on "Let me check what's
    happening and fix all four facts" (141 chars, no report), and doc-paper-ingest ended on a
    self-assessment essay after reading three papers and storing nothing. Every prompt mandates
    a report header; ``report_marker`` names it, and ``run_job`` grants ONE follow-up turn when
    it is absent. Checked over the WHOLE response, not the first line: legitimate reports
    routinely open with a sentence of narration. Silence sentinels bypass the gate.
    """
...
def _strip_report_preamble(job: dict, text: str) -> str:
    ...
    The report gate accepts a header anywhere in the body because real reports routinely open
    with a line or two of narration; that narration is worthless on the phone (the 2026-09-11
    morning briefing and the 2026-09-12 daily trace mining both shipped a working-notes
...
def _run_doc_header(
    job: dict, title: str, job_id: str, prompt: str, *, gate_line: str = "", ledger: str = "",
) -> str:
    """Header of the persisted run document (title, ids, schedule, prompt).

    ``ledger`` (the scheduler's fact-write read-back) goes ABOVE the prompt, not at the end:
    context_from feeds a downstream job the first 8000 chars of this file, and every real
    report runs 14-21K, so an appended block would be truncated away from exactly the consumer
    (the morning briefing) that was repeating the numbers it contradicts."""
    return (
        f"# Cron Job: {title}\n\n"
        f"**Job ID:** {job_id}\n"
        f"**Run Time:** {_hermes_now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        f"**Schedule:** {job.get('schedule_display', 'N/A')}{gate_line}\n"
        f"{ledger}\n"
        f"## Prompt\n\n{prompt}\n\n"
    )
...
def _construct_cron_agent(AIAgent, job: dict, _cfg: dict, setup: _CronAgentSetup, *, workdir, session_id, session_db):
    ...
    return AIAgent(
        ...
        # FORK DIVERGENCE (re-asserted at every upstream sync). Upstream ships skip_memory=False so
        # cron jobs "benefit from (and can update) the user's persistent memory". This fork keeps
        # True: here MEMORY.md is a RENDERED VIEW, rebuilt from category="memory-entry" facts by
        # scripts/render-memory-md.py, by contract "the only writer of the file" — five separate
        # raw-file-edit failure modes landed in two weeks before that contract. The memory TOOLSET
        # stays enabled (fact_store/fact_feedback are how cron writes); only the built-in
        # file-backed memory() is withheld, and the cron prompts tell the model to move on.
        skip_memory=True,
...
def _memory_md_snapshot() -> Optional[dict]:
    ...
    Taken by ``run_job`` right after the session id is minted, and compared against a second
    snapshot at report time by ``_fact_write_ledger`` — the file's tripwire. MEMORY.md may only
    change when the render-memory job materializes it from the store's ``memory-entry`` facts;
    an agent-side edit showing up in any job's ledger line is a violation. Fail-soft.
    """
    try:
        import hashlib
        from hermes_constants import get_hermes_home  # in-function on purpose: tests re-point the constants module
...
def _fact_write_ledger(session_id: str, memory_md_before: Optional[dict] = None) -> str:
    ...
    false in both directions (the 2026-08-15 consolidate run reported four removals that never
    happened while silently hard-deleting a fifth fact its report never mentions; the morning
    briefing then copied those numbers forward). So the scheduler states it instead, after the
    run, from the store — a block the agent cannot author because it is appended once the
    response is already fixed. Attribution is exact: inserts stamp ``facts.source_session`` and
    every mutation stamps ``fact_history.changed_by_session``.

    ``memory_md_before`` is the :func:`_memory_md_snapshot` taken at run start; when given, the
    ...
    if session_id:
        try:
            ...
            logger.debug("fact-write ledger unavailable for %s", session_id, exc_info=True)
    ...
    if db_rows is None:
        ...
        if not (created or created_gone or updated or removed or feedback):
            ...
            if created or created_gone:
                ...
                parts += [f"{r['fact_id']} ({r['category']}, since deleted)" for r in created_gone]
                lines.append(f"- **Created ({len(parts)}):** " + ", ".join(parts))
            if updated:
                lines.append(f"- **Updated ({len(updated)}):** " + ", ".join(str(r["fact_id"]) for r in updated))
            if feedback:
                lines.append(f"- **Feedback ({len(feedback)}):** " + ", ".join(str(r["fact_id"]) for r in feedback))
            if removed:
...
def _fact_write_ledger_rows(session_id: str):
    """The five read-only queries behind :func:`_fact_write_ledger`. Raises on any problem
    (missing db, corrupt file, schema too old) — the caller catches and degrades."""
    ...
    try:
        created = conn.execute(
            "SELECT fact_id, category FROM facts WHERE source_session = ? ORDER BY fact_id",
            (session_id,),
        ).fetchall()
        # A fact this session created and someone then deleted is gone from `facts`; the delete
        ...
        removed = conn.execute(
            ...
            " FROM fact_history WHERE changed_by_session = ? AND op = 'delete' ORDER BY fact_id",
            (session_id,),
        ).fetchall()
        # Trust changes via fact_feedback: record_feedback snapshots op='feedback' since 2026-08-19.
...
def run_job(
    ...
    # MEMORY.md tripwire: paired with a second snapshot at report time by _fact_write_ledger.
    # Taken here (after the pre-run script, before the agent) so everything the agent does to
    # the file is visible.
    ...
    try:
        ...
        # A failed run can still have written facts, so the ledger is emitted on this path too.
        output = (
