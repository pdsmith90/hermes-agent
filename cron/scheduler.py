    ...
    *,
    ...
    try:
        ...
        agent = AIAgent(
            ...
            # FORK DIVERGENCE (re-asserted at the 2026-08-28 upstream sync).
            # Upstream now ships skip_memory=False so cron jobs "benefit from
            # (and can update) the user's persistent memory". This fork keeps
            # True: on this deployment MEMORY.md is a RENDERED VIEW, rebuilt
            # from category="memory-entry" facts by scripts/render-memory-md.py,
            # which is by contract "the only writer of the file". That contract
            # exists because five separate raw-file-edit failure modes landed in
            # two weeks — an oversize append that destroyed the backup, a
            # truncation to 587 bytes mid-run, and a stale write_file rewrite
            # reported as success. Handing every cron agent a working memory()
            # pointed at that file reopens all of them. The memory TOOLSET stays
            # enabled (fact_store/fact_feedback are how cron writes); only the
            # built-in file-backed memory() is withheld, and the cron prompts
            # already tell the model to move on when it reports unavailable.
