...
def _unresolved_delivery_outcome(job: dict, for_failure: bool) -> Optional[str]:
    ...
    if deliver_value in ("local", "none"):
        # local/none jobs don't deliver — an explicit no-delivery choice, not a failure
        # (deliver=none used to log "no delivery target resolved" as an error on every run).
        return None
    if deliver_value == "origin":
        logger.info(
