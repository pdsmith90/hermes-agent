...
def _preflight_check_delivery(job: dict) -> Optional[str]:
    ...
    for lane_value in lane_values:
        for part in lane_value.split(","):
            part = part.strip()
            # "none" is an explicit no-delivery choice, not a platform name; it belongs in this
            # skip-set for the same reason "local" does. Omitting it makes preflight reject the job
            # as an unknown platform — which blocked all four deliver=none jobs on 2026-08-09.
