...
class TestCronMemoryContractOff:
    ...
    def test_skip_memory_is_not_a_per_job_knob(self, tmp_path):
        """No per-job field flips skip_memory: the scheduler always passes the same value.
        ...
        FORK: upstream asserts ``False`` here. This fork constructs cron agents with
        ``skip_memory=True`` on purpose (MEMORY.md is a rendered view that only the
        render-memory job may write — see the comment at the construction site in
        cron/scheduler.py), so the contract this test pins is "not a per-job knob",
        ...
        with _run_job_patches(tmp_path) as (_db, agent_cls):
            run_job(job)
        kwargs = agent_cls.call_args.kwargs
        assert kwargs["skip_memory"] is True  # fork: always True, never read from the job
