...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # Prevent agents from rewriting the rig's own cron schedule through the file tools.
    # ``approvals.deny`` closes the TERMINAL route into cron/jobs.json, but the file tools never
    # consulted it, and the scheduler re-reads jobs.json every tick — so a prompt-injected
    # write_file would recreate the 2026-08-13 incident (a cron run created two live jobs for
    # itself) with no guard at all. The whole cron/ tree is refused: the cronjob tool is the API
    # for schedule changes, and nothing legitimate writes executions.db, the fire locks or the
    # output files through the agent's file tools.
    real_home = _get_real_hermes_home()
    if real_home:
        cron_dir = os.path.join(real_home, "cron") + os.sep
        if any(c.startswith(cron_dir) for c in candidates):
            return (
