...
def _match_user_deny_rule(command: str) -> str | None:
    ...
    for command_variant in _deny_command_variants(command):
        ...
        # A deny glob is written with single spaces ("*hermes cron *"), but the shell treats any
        # whitespace run as one separator — "hermes  cron" ran identically while sliding past the
        # pattern. Test the whitespace-collapsed form too; for a single-spaced command it is the
        # same string, so no existing match changes.
        collapsed = " ".join(candidate.split())
        for pattern in globs:
            lowered = pattern.lower()
            if fnmatch.fnmatchcase(candidate, lowered) or fnmatch.fnmatchcase(collapsed, lowered):
                return pattern
    return None
