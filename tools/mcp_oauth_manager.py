    ...
    async def async_auth_flow(self, request):  # type: ignore[override]
        ...
        try:
            ...
            if resource_lock_released:
                ...
                # MUST precede the aclose() below: that finalization runs the SDK generator's own
                # ``async with self.context.lock`` exit, which releases; with the lock still
                # released here it would push a Semaphore(1, max_value=1) past max and raise.
                import anyio
                with anyio.CancelScope(shield=True):
                    await self.context.lock.acquire()
            # Close the inner generator HERE, in the task that opened it. The SDK holds
            # ``async with self.context.lock`` across its yields (oauth2.py:493), so an inner
            # generator left suspended is finalized later by the GC on a DIFFERENT task, where
            # anyio's Lock.release() raises "The current task is not holding this lock" — and the
            # lock then stays held for the life of the process, so every later auth flow blocks
            # until the connect timeout. httpx closes an auth_flow whenever the request ends early,
            # so this is a common path (it took out `consensus` on every cron run of 2026-08-27).
            # aclose() on an already-exhausted generator is a no-op. Still unfixed upstream.
            await inner.aclose()
