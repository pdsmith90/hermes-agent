    ...
    try:
        ...
        if not resolved_platform:
            with suppress(Exception):
                ...
                # cron/scheduler.py deliberately blanks HERMES_SESSION_PLATFORM, so resolve cron
                # through its own marker (as skill_utils does) or platform_disabled["cron"] is
                # unreachable here and skill_view serves skills the index already withheld.
                resolved_platform = (
...
def skills_list(category: str = None, task_id: str = None) -> str:
    ...
    try:
        ...
        # Annotate pinned skills: the background review fork is told pinned skills are off-limits
        # but had no way to tell which ones are pinned, so it kept composing patches the write
        # guard then refused. Read fresh (pin state lives in .usage.json, outside
        # _find_all_skills' cache signature); load_usage() once, not get_record() per skill;
        # emitted only when True. Same predicate _background_review_write_guard evaluates.
        pinned_count = 0
        try:
        ...
        if pinned_count:
            hint += (
                f". {pinned_count} skill(s) marked \"pinned\": true — autonomous background maintenance "
                "cannot write to those; only the user, in a foreground session, can change them"
            )
        return _json({
...
def skill_view(
    ...
    try:
        ...
        result = {
            ...
            **readiness_extras}
        # Flag a pin here too, not just in skills_list: the background review fork reaches a skill
        # already in play straight from the transcript via skill_view and never lists, so the
        # enumeration-only annotation left that route blind. Emitted only when True.
        try:
            from tools import skill_usage
            if skill_usage.get_record(skill_name).get("pinned"):
                result["pinned"] = True
                result["pinned_note"] = (
                    f"'{skill_name}' is PINNED. Autonomous background maintenance cannot write to it — "
                    "patch and edit are refused, not just delete. Do not compose a patch for this skill; "
                    "pick an unpinned one or report the needed change instead. Only the user, in a "
                    "foreground session, can change it."
                )
        except Exception:
            logger.debug("pinned annotation failed for skill_view(%s)", skill_name, exc_info=True)
