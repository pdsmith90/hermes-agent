...
_INTERPRETER_NAME_RES = tuple((family, re.compile(pattern)) for family, pattern in (
    ...
    # Julia (2026-09-18): a cron experiment-design run used `julia -e 'using Pkg; Pkg.add(...)'` to
    # install into the operator's global environment and passed every detector because julia was
    # not a known interpreter family. Same exec-flag treatment as `python -c`.
    ("julia", r"julia(?:\.exe)?"),
))
...
_INTERPRETER_WITH_ARG = {
    ...
    # julia options that REQUIRE a separate value, so `-t 4 -e ...` still finds -e and `-L helper.jl`
    # does not read the filename as a flag bundle. Optional-value options (-O, -g, --project) are
    # deliberately absent: listing them would swallow a following `-e` as their value.
    "julia": {"-J", "--sysimage", "-L", "--load", "-t", "--threads", "-p", "--procs", "--machine-file",
              "-C", "--cpu-target", "-H", "--home", "--output-o", "--output-ji", "--output-bc",
              "--output-asm", "--output-incremental"},
}
...
def _command_detection_variants(command: str):
    ...
    # Collapsed-whitespace variant: normalization does not squeeze runs of spaces, and every deny
    # glob and dangerous pattern is written with single spaces, so `hermes  cron  create x` matched
    # NOTHING while `hermes cron create x` was blocked. Derived from grep_safe so grep operands stay
    # masked; additive, so it can only produce MORE matches, never fewer.
